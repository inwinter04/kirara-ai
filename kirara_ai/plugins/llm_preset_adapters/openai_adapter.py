...
from kirara_ai.llm.adapter import (
...
async def convert_parts_factory(
    messages: LLMChatMessage, media_manager: MediaManager
) -> list[dict]:
        ...
        for element in elements:
            ...
            for content in element.content:
                if isinstance(content, tools.TextContent):
                    ...
                    output += (
                        f"<media id={content.media_id} mime_type={content.mime_type} />"
                    )
                else:
        ...
        for element in elements:
            if isinstance(element, LLMChatTextContent):
                ...
                parts.append(
                    {
                        "type": "image_url",
                        "image_url": {"url": await media.get_base64_url()},
                    }
                )
                ...
                tool_calls.append(
                    {
                        ...
                        "function": {
                            ...
                            "arguments": json.dumps(
                                element.parameters or {}, ensure_ascii=False
                            ),
                        },
                    }
                )
...
async def convert_llm_chat_message_to_openai_message(
    messages: list[LLMChatMessage], media_manager: MediaManager
) -> list[dict]:
    # gather 必须先包一层异步函数，转化为协程对象， 否侧报错
    results = await asyncio.gather(
        *[convert_parts_factory(msg, media_manager) for msg in messages]
    )
    # 扁平化结果, 展开所有列表
...
class OpenAIAdapterChatBase(
    LLMBackendAdapter, AutoDetectModelsProtocol, LLMChatProtocol
):
    ...
    async def auto_detect_models(self) -> list[ModelConfig]:
        ...
        for model in models:
            ...
            all_models.append(
                ModelConfig(
                    id=model, type=guess_result[0].value, ability=guess_result[1]
                )
            )
        return all_models
